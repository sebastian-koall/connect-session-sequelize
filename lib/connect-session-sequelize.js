...
/**
 * Generates stringified keystream
 * @param {String} stringData - stringified output of encryptData
 * @returns {String}
 */
const decryptData = stringData => {
  const bytes = CryptoJS.AES.decrypt(stringData, secretKey);
  return bytes.toString(CryptoJS.enc.Utf8);
};

/**
 * Generates a 256-bit key (AES-256) and stringifies it
 * @param {String} stringData - JSON stringified data
 * @returns {String}
 */
...
module.exports = function SequelizeSessionInit (Store) {
  class SequelizeStore extends Store {
    ...
    get (sid, fn) {
      ...
      return this.sessionModel
        ...
        .then(function (session) {
          ...
          if (this.options.encrypted) sessionData = decryptData(sessionData);
    ...
    set (sid, data, fn) {
      ...
      if (this.options.encrypted) stringData = encryptData(stringData);
