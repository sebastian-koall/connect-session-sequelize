...
module.exports = function SequelizeSessionInit(Store) {
  class SequelizeStore extends Store {
    constructor(options = {}) {
      ...
      if (options.table) {
        debug(options, `Using table: %s for sessions, ${options.table}`);
        ...
        debug(options, 'No table specified, using default table.');
    ...
    // Modified ---
    get(sid, fn) {
      debug(this.options, `SELECT "%s", ${sid}`);
      return this.sessionModel
          .findOne({ where: { sid } })
          .then(session => {
          if (!session) {
        debug(this.options, `Did not find session %s, ${sid}`);
        return null;
      }
      debug(this.options, `FOUND %s with data %s, ${sid}, ${session.data}`);
    ...
    // Modified ---
    set(sid, data, fn) {
      debug(this.options, `INSERT "%s", ${sid}`);
    ...
    touch(sid, data, fn) {
      debug(this.options, `TOUCH "%s", ${sid}`);
      if (this.options.disableTouch) {
        debug(this.options, `TOUCH skipped due to disableTouch "%s", ${sid}`);
        return fn();
      }
    ...
    destroy(sid, fn) {
      debug(this.options, `DESTROYING %s, ${sid}`);
      return this.sessionModel
        ...
        debug(
          this.options,
          `Session not found, assuming destroyed %s, ${sid}`,
        );
        return null;
      }
    ...
    clearExpiredSessions(fn) {
      debug(this.options, 'CLEARING EXPIRED SESSIONS');
      return this.sessionModel
